...
class Channels {
    ...
    createItem(channel, message, title, url, imagesIds, type) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this._rest.publishMessage(channel.id, message, title, url, imagesIds, type).then((status) => {
                ...
                resolve(Object.assign({ "publishResult": status }, ErrorManager_1.ErrorManager.getErrorManager().OK));
            }).catch((err) => {
    ...
    _onChannelMyAppreciationReceived(my_appreciation) {
        this.fetchChannel(my_appreciation.channelId).then((channel) => {
            let appreciationObj = {
                "appreciation": Channel_1.Appreciation.None,
                "channel": channel,
            ...
            switch (my_appreciation.appreciation) {
                case "applause":
                    appreciationObj.appreciation = Channel_1.Appreciation.Applause;
                    break;
                case "doubt":
                    appreciationObj.appreciation = Channel_1.Appreciation.Doubt;
                    break;
                case "fantastic":
                    appreciationObj.appreciation = Channel_1.Appreciation.Fantastic;
                    break;
                case "happy":
                    appreciationObj.appreciation = Channel_1.Appreciation.Happy;
                    break;
                case "like":
                    appreciationObj.appreciation = Channel_1.Appreciation.Like;
                    break;
                case "none":
                    appreciationObj.appreciation = Channel_1.Appreciation.None;
                    break;
            }
     ...
     */
    likeItem(channel, itemId, appreciation) {
        if (!channel || !channel.id) {
     ...
     */
    getDetailedAppreciations(channel, itemId) {
        if (!channel || !channel.id) {
